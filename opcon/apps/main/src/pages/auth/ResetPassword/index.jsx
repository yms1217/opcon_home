...
import { resetPasswordComplete } from '@repo/apis'
...
function ResetPassword() {
  ...
  const handlePassError = (resultCode, param) => {
    switch (resultCode) {
      case 'USER_40004':
      ...
      case 'BAD_REQUEST':
        if (param && Array.isArray(param)) {
          setErrorMessage(param[0].message)
        } else {
  ...
  const formDisabled = !hasPasswordToken || !isTokenValid
  ...
  const onSubmit = async (data) => {
    ...
    setErrorEmail('')
    try {
      const response = await resetPasswordComplete(passwordToken, data.email, data.pw)
      ...
      handlePassError(error.response?.data?.errorCode, error.response?.data?.validationErrors)
      setIsLoading(false)
    }
  }
      ...
      <LoginContainer>
        ...
        <LoginBox>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {errorEmail && <ErrorMessage className="typographyBody6">{errorEmail}</ErrorMessage>}
